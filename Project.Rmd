...
* Predictor variables relationships: 
    - Some predictors have strong relations with one another: rm has a strong negative relationship with lstat but a strong positive one with medv. medv and lstat also have a strong/moderate negative relationship
...
The resulting coefficients for the selected predictors is shown in table \@ref(tab:best-sub-coef), and seems to show nox and rad as the strongest positively influencing predictors. This lines up with nox and rad positive strength in the first PC in the PCA analysis and their strong positive correlation. The same can be observed with the strongest negative component (zn), which also were strong in PC1 and had strong negative correlation with lcrim . 

### Ridge Regression 

Ridge Regression uses a modified loss function to add a penalty to large coefficients to improve predictive solution (constraints variance). Ridge Regression also uses scaled data like Best Fit to avoid coefficients dominating due to scale differences.

#### Lambda Choice 

Lambda decides how much the algorithm penalties large coefficient, the more it does so the more the values slope down as seen in figure \@ref(fig:ridge-fit-lambdas). Most of the values seems clustered next to each other, except rad(8), nox(4) and zn, with rad especially dominating the coefficient sizes before dropping next to lambda 0. Nonetheless, to know which lambda choice would make the most sense, a 10 fold cross validation is carried in figure \@ref(fig:ridge-fit-cv) and shows that the MSE start low and only starts to pick pace and go up once it crosses $log(lambda) = 0$. The exact minimum MSD is not clear from the graph, but using the "lambda.min" field returns a ~0.006 minimum (correct to 3DP) for the lambda also as seen in figure's \@ref(fig:ridge-fit-cv) code.

#### Selected Lambda Coefficent Discussion

As seen in figure \@ref(fig:ridge-fit-lambdas), nox(7), zn(1) and rad(8) seemed to be the most dominant coefficients and this can also be observed in the exact values with ideal lambda (~0.006) seen in table \@ref(tab:ridge-fit-coef). The coefficients selected seem similar to the coefficients seen with best subset selection, but with slightly different values due to the adjusted algorithim and no subsetting (all ps). Nonetheless, just like the Best Subset coefficient, the values most correlated to lcrime and the strongest in PC1 generally being the most dominant as discussed with the Best Subset coefficients. 

### LASSO

LASSO is quite similar to Ridge Regression, however unlike ridge regression which always gets all p variables LASSO can subset (coefficients can become zero). This is because of the modified $SS_E$ which takes the absolute value instead of the square of each coefficient. LASSO also uses scaled data like like the other techinques to avoid coefficients dominating due to scale differences.

#### Lambda Choice

Similarly to Ridge Regression, lambda decides how much the algorithm penalises large coefficients. The values of coefficients against lambda can be seen in \@ref(fig:lasso-lambdas), the dominant coefficients are similar to Ridge, but some of the coefficient eventually converge to zero. Nonetheless, as with Ridge Regression, to find the ideal lambda value MSE in a 10 fold cross validation is used, and as seen in \@ref(fig:ridge-fit-cv). The shape of the error plot is quite similar, but this time it returns a minimum ~0.0163.

#### Selected Lambda Coefficent Discussion

The values for the coefficients are quite similar to Ridge Regression coeficients disincluding 0.00 predictors, but since LASSO's main difference with Ridge regression is its ability to subset the predictors they will be the main concern of this discussion. It seems LASSO decided to subset tax, medv, rm and chas (at least correct to 3 DP), which are the same as the ones subsetted by the Best Fit (with 9 predictors), which as stated in its analysis are subsetted for being weakly correlated with lcrim (chas) or being correlated with other predictors that can represent them (tax, rm and medv).

## Model Evaluation Using k-fold Cross Validation and Best Model

To evaluate all the models using cross validation, it is important to ensure that they use the same folds for the cross validation. To do this, the indexes for the folds for subset cross validation (using 10 folds) are passed to the regularisation fittings (LASSO and Ridge). The results are presented in Figure \@ref(fig:cv-comparisons). 

As seen in figure \@ref(fig:cv-comparisons) the regularisation methods (LASSO and Ridge Fit) seems to perform significantly better than the subsetting method (Best Fit). This is probably because while Best Fit selects the best set of predictors for a given model size, its larger coefficient leads to larger variation in the estimator weakening the predictions. 

When it comes to the regularisation methods, LASSO seems to perform slightly better because of subsetting out uneeded predictors. Moreover, since LASSO requires less predictors it should be more convienent and easier to use and hence is my choice of all the three methods. LASSO had subsetted out chas, rm, tax and medv as stated before either because they had poor correlation with the response (lcrim) or could be represented by other predictors (rm, tax and medv). 
...
errors = c(lassoCv$lambda.min, ridgeFitCv$lambda.min, bssMse[9])
