...
The resulting coefficients for the selected predictors is shown in table \@ref(tab:best-sub-coef), and seems to show nox and rad as the strongest positively influencing predictors. This lines up with nox and rad positive strength in the first PC in the PCA analysis and their strong positive correlation. The same can be observed with the strongest negative component (disf), but not with age surprisingly (one of the strong negative coefficients in PC1). This is probably because not all variation it causes leads to variation in lcrim specifically (after all it has lower absolute correlation compared to rad and nox) and because perhaps it can be represented by other predictors (disf has moderate/strong correlation to it).
...
This section contains all supplementary material and is divided into three sections (Tables, Plots and "Abbreviations and Short hands"). The code required to generate the supplementary material is also included
