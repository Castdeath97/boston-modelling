...
To summarise, it seems that the relationships suggest that there are a couple of variables that might have a strong impact to model (e.g. rad or tax). Moreover, the relationships also suggest that many will be subsetted due to relationships that can be represented with other variables (e.g. rad for tax, or vice versa) or because of very poor relationships with all variables (e.g. chas).
...
As seen in figure \@ref(fig:cv-comparisons) the regularisation methods (LASSO and Ridge Fit) and the subsetting method (Best Fit) seem to perform similarily for the given data. Best Fit seems to be the best, then LASSO then Ridge Fit. 

Nonetheless, the regularisation methods would be the better bet, since Best Fit is incredibly slow due its exhaustive methods and the advantage is that it provides in exchange is small. For the regularisation methods, since LASSO requires less predictors it should be more convienent and easier to use and hence is my choice of out of the two. LASSO had subsetted out chas, rm, tax and medv as stated before either because they had poor correlation with the response (lcrim) or could be represented by other predictors (rm, tax and medv). 
